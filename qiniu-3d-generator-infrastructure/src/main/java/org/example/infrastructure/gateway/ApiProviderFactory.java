...
public class ApiProviderFactory {
    ...
    @Value("${api.provider.default:meshy}")
    private String defaultProvider;
    
    @Value("${api.provider.fallback:hunyuan}")
    private String fallbackProvider;
    
    @Value("${api.provider.enable-fallback:true}")
    ...
    public String textTo3D(String text, Map<String, Object> params, String provider) {
        try {
            switch (provider.toLowerCase()) {
                ...
                default:
                    throw new IllegalArgumentException("不支持的API提供商: " + provider);
            }
        } catch (Exception e) {
    ...
    public String[] getAvailableProviders() {
        return new String[]{"meshy", "hunyuan"};
    }
